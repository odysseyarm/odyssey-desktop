...
pub fn use_tray_menu(_cancellation_token: tokio_util::sync::CancellationToken) -> TrayIcon {
    ...
    // We use "Close" as default text because we start the window visible
    // FIXME https://github.com/DioxusLabs/dioxus/issues/4495
    ...
    let _ = menu.append_items(&[&PredefinedMenuItem::quit(None)]);
    ...
    let tray = provide_context(builder.build().expect("tray icon builder failed"));
